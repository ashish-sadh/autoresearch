...
| d32 431h | 0.870 | same | Sky derails into mood/psychology; robot truncated | Sky starts on light/optics then pivots to mental health "skybox"; math: garbled equation; robot: cuts off early | New best val_bpb (1.8% gain); metric improved significantly but chat quality regressed — diagnosed as SFT recipe over-sharpening end_id |
| d32 531h | 0.863 | better | Restored full multi-paragraph; numbered Q&A | Sky: photons/light/visible spectrum (correct domain); robot: self-reflective first-person; math: formula derivation attempt | New best + chat restored: gentler SFT (300 steps lr=5e-5) preserves chat quality on stronger base |
